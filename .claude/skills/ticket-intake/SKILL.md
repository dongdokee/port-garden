...
- Infer top ticket type with rationale and list 1-2 alternatives.
- Show concise evidence from the prompt.
- Ask user to approve or override.
- On approval, lock the selected type and load type-specific MRI requirements from `references/mri-matrix.md`.

### Phase 2: Completeness Audit
- Classify each MRI field as `clear`, `unclear`, or `missing`.
...
- Present audit summary and get user approval before proceeding.
...
When multiple fields are unresolved, pick the highest-impact gap from either track.

**Track A — Intent** (what to build and why):
1. Safety and impact-critical fields (security scope, affected users, severity, regression risk)
2. Feature foundation fields (User Stories, AC boundaries, expected outcome)
3. Scope control fields (`non-goals`, constraints)
4. Interaction and quality fields (`design specs`, `edge cases`)

**Track B — Location** (where to build it):
1. Entry point and target files
2. Related paths and impacted modules
3. Existing suite path, artifacts, and references

Interleave across tracks; do not exhaust one before starting the other.
